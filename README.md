...
## Deployment
This project uses GitHub Actions to automate deployments to the GoDaddy hosting environment using **GitHub Environments** (`development` and `production`).
...
- **Workflow:** The workflow prepares the SSH key, bundles the codebase (excluding local settings and development files), uploads the archive to GoDaddy via `scp`, and extracts it on the host via `ssh`.
...
|---|---|
| `REMOTE_HOST` | The server host (IP address or domain) of your GoDaddy hosting for that environment. |
...
Since `settings.php` is ignored by Git, it is not bundled or overwritten during deployment. Make sure you manually create/maintain the appropriate environment-specific `settings.php` file on your GoDaddy folders (`dev` and `production` paths).